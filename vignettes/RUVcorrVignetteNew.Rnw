...
\section{Gene prioritization}
One of the methods that can be applied given a cleaned version of the dataset is gene prioritization. Gene prioritization identifies candidate genes that are likely to be involved in the same biological pathways or related pathways than a set of known genes. The gene prioritization method in this package is very similar to the approach described in the paper by Oliver et al \cite{Oliver}. For demonstration purposes assume that the following genes involved in the synaptic vesicle cycle are in fact candidates: 
<<>>=
...
In order to prioritize genes, typically a correlation threshold is determined. The absolute values of correlations between genes that exceed this threshold are considered to be truly co-expressed. Here, we use a threshold that corresponds to an empirical false discovery rate (EDFR) of 0.3. However, this requires the estimation of EDFR for all possible thresholds. This can be achieved using the function \Rfunction{calculateEFDR}: 
<<>>=
efdr <- calculateEFDR(CleanData, exclude=c(nc_index, cand_index), 
                      index.ref=na_index, set.size=length(cand_index), 
                      Weight=NULL)
threshold <- predict(efdr$loess.estimate, 0.3)
threshold
@
\bioccomment{It is important to exclude genes that could bias the estimation of the EFDR.}

The EFDR can also be visualized using (see Figure \ref{figure/EFDR-1}):
<<EFDR, fig.show='hide', fig.width=10, fig.height=6, results='hide', warning=FALSE>>=
plotEFDR(efdr)
@

\incfig{figure/EFDR-1}{1\textwidth}{EFDR for every possible threshold.}

\subsection{Prioritizing candidate genes}
Having determined the threshold we can use the function \Rfunction{prioritise} in order to establish whish candidates are also likely to be involved in the sodium-channel:
<<>>=
prior<-prioritise(CleanData, na_index, cand_index, Weight=NULL, threshold=threshold)
print(prior)
xx[which(is.element(names(xx), prior[,1]))]
@
This analysis prioritizes SNAP25 and CACNA1A.
